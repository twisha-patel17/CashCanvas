...
export const getBudget = async (req, res) => {
    try {
        ...
        const exceededAmount = remainingBudget < 0 ? Math.abs(remainingBudget) : 0;
...
export const getBudgetHistory = async (req, res) => {
    try {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const budget = categories.reduce((acc, curr) => acc + curr.monthlyBudget, 0);
            const expense = expenses.reduce((acc, curr) => acc + curr.amount, 0);
            const remaining = budget - expense;

            if (expense > 0 || targetDate.getMonth() === currentDate.getMonth()) {
                history.push({
...
export const getBudgetInsights = async (req, res) => {
    try {
        ...
        // NOTE: added month/year query support here too, mirroring getBudget,
        // since the response below reports a month/year — remove this block
        // if you intended insights to always be the current month only.
        const month = Number(req.query.month);
        const year = Number(req.query.year);
        ...
        const categoryBudgets = categories.map((category) => {
            const spent = expenses
                .filter(
                    (expense) =>
                        expense.category?._id.toString() === category._id.toString()
                )
            ...
            const exceededAmount = Math.max(spent - category.monthlyBudget, 0);
            const isExceeded = spent > category.monthlyBudget;
